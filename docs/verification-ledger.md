...
## 2026-09-11 — Live E2E round 6 (.env re-untrack R6-1, header search typeahead R6-2, admin gate E2E R6-3, PAD/traceability alignment R6-4)

Environment: fresh clone @ `316befa` (main). Embedded PostgreSQL 17.5 (zonky binaries, port 5432, trust auth, migrate+seed) — real-PG integration suites + full Playwright suites executed locally against the production build, plus live-origin sweeps of both deployments. Plan + findings: `docs/plans/2026-09-11-live-e2e-remediation-round6.md`.

| Slice | Fix | Evidence | Label |
|---|---|---|---|
| **Live audit round 6** | — | Repo Playwright suite vs live storefront origin: **46/46 PASS** (round-5 fixes R5-2/R5-3 confirmed deployed). 31-route storefront sweep + 8-route admin sweep + security headers + robots/sitemap parity + canonical/og:url + JSON-LD + typeahead contract + TTFB (~140 ms vs the 800 ms SLO) all healthy; browser probes of checkout journey, PDP `?variant=` deep-link, mini-cart drawer, admin sign-in error copy, newsletter form, collections | **Verified** |
| **R6-1 .env re-untracked (P0)** | `git rm --cached .env` — commit `316befa` ("new start server log") had re-tracked the 51-line `.env` with real `BETTER_AUTH_SECRET` + `CRON_SECRET` values, the exact C2/R4-1 shape | RED: `git ls-files` showed `.env`; the CI secret-scan pattern executed verbatim locally flagged `.env:7` + `.env:31`. GREEN: CI-checkout simulation (working-tree `.env` set aside) exits clean. Working-tree file preserved for local servers; `.gitignore:13` already covers it. **Rotation of both secrets is a required ops action (values rode a public push)** | **Verified** |
| **R6-2 header search typeahead** | New `lib/search-suggest.ts` (pure shaping: 2-char route minimum, 10-result cap, malformed-payload tolerance) + `components/search-trigger.tsx` (ARIA 1.2 combobox, 250 ms debounce, abortable fetches, ArrowUp/Down/Enter/Escape, bare submit → `/search?q=`, derived dropdown visibility — zero effect-body setState) + header mount (desktop) + mobile drawer Search link + `commerce/catalog.searchTypeaheadCategories` wired into the typeahead route (Zod-tightened `{slug,title}` groups; journal group stays reserved until the FR-703 reader route ships — a live link to it would 404) | TDD red→green: 6 unit assertions + 7 E2E specs RED vs the pre-fix local build (7/7 failed, incl. `searchbox`→combobox role lesson); GREEN **53/53** local full suite; `react-hooks/set-state-in-effect` lint error caught mid-slice and eliminated via derived state (final lint 8/8). Journal-link hazard documented in-code and in traceability | **Verified** (unit + local E2E; live RED n/a — feature absent on live until redeploy) |
| **R6-3 admin gate E2E** | `apps/admin/e2e/admin-gate.spec.ts` (8 credential-free specs: unauth 307s preserve `?redirect=` for `/`, `/orders`, `/products`, `/customers`, unknown paths; sign-in form contract; §9.3 headers on `/sign-in`; wrong-credentials typed error + gate still holds) + `playwright.config.ts` mirroring web (prod-build webServer, `:3001`, `E2E_BASE_URL` retarget) + `e2e` script + CI "E2E admin (Chromium)" step | GREEN **8/8 vs the local prod build AND 8/8 vs the live admin origin** — the live gate contract is pinned against regression for the first time (previously only manual curl probes) | **Verified** (local + live) |
| **R6-4 doc alignment** | PAD §11: `R-SEO-1` (sitemap/robots) and `R-SEO-2` (/search page) rows flipped Open→Resolved with round-4 commit + round-6 live evidence; facet-indexing row marked blocked-by-R-SHOP-2. `docs/traceability.md`: FR-104 Deferred→Aligned (round 6), FR-101 header note, FR-106 spec count. Round-6 plan + this entry + session narrative | PAD rows previously claimed "Open — P0" for surfaces that shipped in `180cc54` and were live-verified this round — evidence-labeling discipline restored | **Verified** |

**Non-findings (investigated, recorded to prevent re-chasing):**
- CI `branches: ain]` in `ci.yml` is a terminal display artifact of `[main]` (the same bracket-eating artifact session 10 documented for git output) — the YAML parses `['main']` and push-triggered CI is configured correctly.
- PDP "Dimensions" accordion absent on the Halden PDP: data-driven (FR-701 hide-when-missing); the accordion renders only when the product carries dimension content.
- Static pages outside FR-704's seeded set (sustainability/showrooms/trade/cookies/accessibility): admin-managed content (FR-809); the footer only links pages that exist — no silent 404 links.
- Local-only canonical/og:url spec failures during this round were an env artifact (`.env`'s production `NEXT_PUBLIC_SITE_URL` inlined at build time); CI sets the local origin. Restarting the local build with the CI value produced 53/53.

**Gates after round 6:** `pnpm lint` 8/8 · `pnpm typecheck` 8/8 · `pnpm test` 7/7 tasks (integration suites live on PG 17.5) · `pnpm build` 2/2 · Playwright web **53/53** local · admin **8/8** local + 8/8 live.

**Ops actions required (cannot be executed from this sandbox):**
1. **Rotate `BETTER_AUTH_SECRET` and `CRON_SECRET`** — `316befa` re-exposed both on a public repo (R6-1 untracks them; history keeps the bytes). The cart-cookie HMAC rides `BETTER_AUTH_SECRET`, so rotation restarts the 30-day guest-cart window — acceptable post-exposure cost.
2. Redeploy both apps via `./start_server.sh` to publish R6-2 (header search) — the storefront's canonical/og:url surfaces from round 5 are already live.
3. Carried from rounds 3–5: Stripe env alignment, junk UUID-shaped cart-row cleanup.

## 2026-09-11 — Live E2E round 7 (.env 4th re-untrack R7-1, FR-703 journal reader R7-2, FR-704 dozen static pages R7-3, search depth R7-4/R-DB-1/R-SHOP-1, PAD hygiene R7-9)

Environment: fresh clone @ `a5ffbf7` (main). Embedded PostgreSQL 17.5 (zonky binaries via `@embedded-postgres/linux-x64@17.5.0-beta.15`, port 5488, trust auth, pgcrypto/pg_trgm/citext, migrate+seed) — real-PG integration suites + full Playwright suites executed locally against the production build, plus live-origin sweeps of both deployments. Plan + findings: `docs/plans/2026-09-11-live-e2e-remediation-round7.md`.

| Slice | Fix | Evidence | Label |
|---|---|---|---|
| **Live audit round 7** | — | Repo Playwright suite vs live storefront origin: **53/53 PASS** (round-6 redeploy confirmed — the checked-in `start_server_log.txt` health checks match; header search live). Admin gate suite vs live admin origin: **8/8 PASS**. Beyond the repo suite, a 36-probe diagnostic sweep (facets, sort/pagination, search depth, journal, static pages, typeahead edges, newsletter, account gate, TTFB, headers, JSON-LD, mobile) surfaced 9 findings — every one validated against the code before classification | **Verified** |
| **R7-1 .env 4th re-untrack (P0) + R7-1b scan-red prose** | `git rm --cached .env` — commit `a5ffbf7` ("update start server log") re-tracked the 51-line `.env` with real `BETTER_AUTH_SECRET`/`CRON_SECRET` (4th occurrence: C2 → R4-1 → R6-1 → R7-1). Plus `docs/session_8.md:81` quoted the literal OpenSSH key header in prose — the CI scan's private-key-header pattern flags it even on a clean checkout | RED: exact CI scan replay flagged `.env:7`, `.env:31`, `session_8.md:81`. GREEN: after untrack + prose rephrase ("OpenSSH private-key header"), a `git archive`-of-staged-index checkout simulation exits **clean** (verified against the index tree, not just the working tree — working-tree `.env` still trips `--no-ignore` scans by design). Rotation of both secrets remains a **required ops action** (4th public exposure) | **Verified** |
| **R10-6/R10-6b .env 5th exposure (P0) + de-marker of the docs that quote the scan pattern** | `git rm --cached .env` — commit `fc0379a` ("update start server log") re-tracked the 51-line `.env` with real `BETTER_AUTH_SECRET`/`CRON_SECRET` (5th exposure: C2 → R4-1 → R6-1 → R7-1 → fc0379a). Round 11 additionally de-markers `AGENTS.md:75` + the R7-1 ledger row itself — both quoted the private-key header marker in prose while documenting the R7-1b rule, so the CI scan stayed red on a clean checkout and masked the regression | RED: exact CI scan replay flagged `.env:7`, `.env:31`, `AGENTS.md:75`, `verification-ledger.md:341`. GREEN: after untrack + prose rephrase (describe the OpenSSH private-key header line, never reproduce it), the `git archive`-of-staged-index checkout simulation exits **clean**. Rotation of both secrets remains a **required ops action** (5th public exposure) | **Verified** |
| **R7-2 FR-703 journal reader route (HIGH)** | `apps/web/src/app/journal/[category]/[slug]/page.tsx` (RSC, sanitized bodyHtml, Article + BreadcrumbList JSON-LD via `safeJsonLd`, canonical/og:url via `publicPageMetadata`, related-product embeds via `listProducts({ids})` — hide-when-missing) + index links + `commerce/catalog.getJournalPost`/`searchTypeaheadJournal` + sitemap journal entries (`buildSitemapEntries` priority 0.4) + typeahead journal group (category-scoped hrefs with flat fallback) + dropdown Journal kind label | TDD red→green: 7 integration specs (`catalog-journal.integration.test.ts` — published/unknown/unpublished, category validation, typeahead journal, sitemap entries) + 3 unit specs (`seo.test.ts` journal URLs; `search-suggest.test.ts` category-scoped href + tolerance fallback) + 7 E2E specs (`journal-flows.spec.ts` 5 + search/seo extensions) — all captured RED first (route 404'd, functions absent). Two real mid-slice lessons: the option `<li role=option>` navigates via mousedown (no anchor — spec asserts click-through, not href); E2E needed a fresh build after the client-island label change (webServer `reuseExistingServer` serves stale builds) | **Verified** (integration + unit + local E2E) |
| **R7-3 FR-704 dozen static pages (MEDIUM)** | Seed rows for the six missing pages (sustainability, materials, showrooms, trade-program, cookies, accessibility) — idempotent `onConflictDoNothing`, honest copy (trade-program names the manual concierge path while FR-901 self-service is in preparation; cookies states the Phase-2 consent-vendor posture instead of claiming a banner that does not exist) | TDD red→green: `packages/db/src/seed-coverage.test.ts` (12 published slugs + FR-703/FR-105 seed companions) RED (6 missing) → GREEN after re-seed; E2E "static pages — FR-704 full dozen" asserts all twelve 200 + the accessibility WCAG 2.2 AA copy | **Verified** |
| **R7-4 search depth — R-DB-1 + R-SHOP-1 (HIGH)** | Migration `0001_faulty_warpath.sql` (additive): `immutable_text_array_to_string` preamble (Postgres marks `array_to_string` STABLE — for text[] output is deterministic, so the wrapper honestly re-declares immutability) + `product.search_vector` GENERATED ALWAYS STORED (title weight A, materials+desc weight B per PRD §8.8) + GIN index; backfills all rows. `listProducts`/`searchTypeahead` now probe the maintained vector with `expandSearchTerms` (pure, `commerce/src/search-terms.ts`) unioning `search_synonym` expansions + `similarity(p.title, q) >= 0.5` trigram + ILIKE | TDD red→green: schema-shape spec (generated ALWAYS/STORED + GIN declared), 7 `expandSearchTerms` unit specs, 5 `search-depth.integration.test.ts` specs (fixture sofa found via couch→sofa, typeahead parity, 'halden linnen armchair' typo → Halden at similarity 0.87, exact FTS, DDL assertions) — all RED first. Two failed-generation lessons root-caused en route: `to_tsvector('english', …)` is not immutable without the `::regconfig` cast; `array_to_string` is STABLE. Trigram honestly measured: partial two-word typo "halden linnen" = 0.478 < 0.5 (stays unmatched by design), full-title typo = 0.870 | **Verified** (integration + unit + local E2E) |
| **R7-9 PAD hygiene (INFO)** | PAD v1.1: revision-history entry for rounds 6–7 (fixes the missing-entry drift), TOC §11 heading now matches the body ("Known Issues & Outstanding Tasks"), §4.2 money citation ADR-7 → ADR-001, Last Updated 2026-09-11, §11 rows updated (R-DB-1/R-SHOP-1 Resolved with commit refs; R-SHOP-2's stale "sort falls to default" corrected — `?sort=` URL sorting verified working; R-DB-2 explicitly deferred with rationale) | Round-7 sweep evidence for each row; the sort correction verified live (`/shop?sort=price_asc` honored) and in SQL (`catalog.ts` orderBy branches) | **Verified** |

**Non-findings (investigated, recorded to prevent re-chasing):**
- D2/D3 sweep reads: sort DOES work via URL (`?sort=newest|price_asc|price_desc` honored in SQL and live); pagination is wired but the 6-product catalog never crosses the 24/page threshold — 0 `?page=` links is correct behavior, not a gap.
- D10 "no SKU markers on PDP": FR-302 `?variant=SKU` deep-link IS implemented (`product-buy-panel.tsx` reads and reflects it via replaceState) — the probe looked for data attributes the component doesn't emit.
- Typeahead `?q=%00%01bad` → 200: Zod `.min(2)` maps sub-minimum/garbage queries to the empty shape by design (graceful client contract, pinned since round 6).
- Reviews absence on PDP (E5-fix) is R-SHOP-3's known open queue item (verified-buyer gate + moderation + no seeded reviews), not a silent regression; PDP JSON-LD AggregateRating correctly renders only when `ratingCount > 0`.
- Consent banner absence is the PRD §4.8 Phase-2 soft-launch posture (local internal provider); the FR-704 cookies page now states it honestly.

**Gates after round 7:** `pnpm lint` 8/8 · `pnpm typecheck` 8/8 · `pnpm test` 7/7 tasks (commerce 22 files/146 tests incl. 12 new; db 4 files/22 tests incl. 4 new; web 35 incl. 2 new; integration suites live on embedded PG 17.5) · `pnpm build` 2/2 · Playwright web **64/64** local (53 prior + 5 journal + 2 static-pages + 3 search-depth + 1 typeahead-journal) · admin **8/8** local.
...
## 2026-09-11 — Live E2E round 8 (checkout placeholder-key honesty R8-1, homepage journal links R8-2, PLP breadcrumb JSON-LD R8-3, footer newsletter/social R8-4, FR-701 sections + review seeds + aggregateRating R8-5, back-in-stock notify R8-6, dismissible announcement R8-7)

Environment: fresh clone @ `d94c020` (main). Embedded PostgreSQL 17.5 (zonky binaries via `@embedded-postgres/linux-x64@17.5.0-beta.15`, port 5488, trust auth, pgcrypto/pg_trgm/citext, migrate+seed) — real-PG integration suites + full Playwright suites executed locally against the production build, plus live-origin sweeps of both deployments. Plan + findings: `docs/plans/2026-09-11-live-e2e-remediation-round8.md`.

| Slice | Fix | Evidence | Label |
|---|---|---|---|
| **Live audit round 8** | — | Repo Playwright suite vs live storefront origin: **64/64 PASS** (round-7 redeploy confirmed — the 10 awaiting-redeploy deltas from session_12 are closed: journal routes, static pages, search depth all live). Admin gate suite vs live admin origin: **8/8 PASS**. Beyond the repo suite, a 36-probe diagnostic sweep (homepage sections, PDP surface, cart, footer, journal, search, JSON-LD, headers, TTFB 109–142 ms, checkout money path) surfaced 8 findings — every one validated against the code before classification; probe artifacts (P13 promo-field case sensitivity, P33 `.or()` masking, P04 sort-group false positive) disproven and recorded | **Verified** |
| **R8-1 checkout placeholder-key honesty (HIGH, money path)** | New `apps/web/src/lib/stripe-config.ts` `isStripePublishableKeyConfigured()` — client now mirrors the server `getStripe()` `set-me` sentinel rule; `createPaymentIntentAction` maps `STRIPE_NOT_CONFIGURED` to a customer-safe message and logs the operator detail server-side (env instructions never leave the server); `CART_EMPTY` messages stay customer-meaningful (pinned) | Live probe confirmed the defect end-to-end: address form rendered on the placeholder build, submit leaked "Stripe is not configured: set STRIPE_SECRET_KEY in .env". TDD red→green: 4 unit specs (helper incl. server-rule parity) + 2 action-mapping specs (no-leak + CART_EMPTY pin) + E2E "checkout treats placeholder publishable keys as unconfigured (R8-1)" — honest notice renders, no address form. Commit `2402ac1` | **Verified** |
| **R8-2 homepage journal preview links (MEDIUM)** | `JournalPreview` cards are now `next/link` to `/journal/{category}/{slug}` (R7-2 URL contract) | TDD red→green: 2 E2E specs (`journal-flows.spec.ts` — href shape for both seeded posts + click-through 200). Commit `c663028` | **Verified** |
| **R8-3 PLP breadcrumb JSON-LD (MEDIUM)** | `/shop` + `/shop/{category}` emit `breadcrumbJsonLd` via `safeJsonLd` (PDP escaping idiom); category trail capitalizes the visible segment to match h1 + JSON-LD name | TDD red→green: 2 E2E specs (`seo-flows.spec.ts` — BreadcrumbList on /shop and /shop/seating, trail names, absolute URLs). Commit `fdbd58b` | **Verified** |
| **R8-5 FR-701 §6/§7/§9 + review seeds + aggregateRating (MEDIUM)** | `listApprovedTestimonials` (approved-only, newest-first, bounded, product-joined); 7 existence-guarded approved review seeds across 5 products (`isVerifiedPurchase` honestly false — no orders back demo rows); homepage §6 brand-story teaser (→ /our-story), §7 materials 3-cards (→ /materials), §9 testimonials 3-up; PDP `aggregateRating` renders when `ratingCount > 0` | TDD red→green: 2 integration specs (`testimonials.integration.test.ts` — approved-only, recency, bound) + seed-coverage spec (≥6 approved across ≥4 products) + 2 E2E specs (homepage sections render; PDP aggregateRating + visible reviews). Commit `c59ce62` | **Verified** |
| **R8-4 footer newsletter + social (MEDIUM)** | `NewsletterForm` gains `source`/`idPrefix`/`compact` (distinct DOM ids; `newsletter_subscriber.source` records origin); footer carries the compact form + three aria-named social links (`noopener noreferrer`) | TDD red→green: 1 E2E spec (footer form labeled + valid submit surfaces status + all three social names). Commit `33155a6` | **Verified** |
| **R8-6 back-in-stock notify (MEDIUM)** | `commerce/back-in-stock.ts` `requestBackInStock` (typed unknown-variant failure; `onConflictDoNothing` dedupe on the existing unique index) + `notifyBackInStockAction` (Zod, 3/hour/IP newsletter idiom, customer-safe mapping) + PDP Notify-me row per sold-out variant (disabled swatch/selection contract unchanged) + seed adds sold-out Rust variant (additive natural key; 0 on hand vs safety 2 stays `out_of_stock` given the throw's ≤7-day lead time) | TDD red→green: 2 integration specs (register → `registered`, repeat → `already_registered` with 1 row; unknown variant rejects) + 4 action specs (validation, outcome mapping, rate-limit mapping, unknown-variant mapping) + 1 E2E spec (disabled Rust swatch, labeled form, registered status, dedupe status). Two spec-robustness lessons recorded: run-unique email (dedupe collision) and documented fixture reset of the loopback `back_in_stock` rate-limit bucket (repeated dev-loop runs share one "unknown" identity; CI runs once; the guardrail's integration suite still pins the contract). Commits `49a7683`, `528e49c` | **Verified** |
| **R8-7 dismissible announcement (LOW)** | `announcement-store` (Zustand persist, keyed on the announcement id — new content re-shows) + `AnnouncementBar` client island; visibility derives via `useSyncExternalStore` (M-3 idiom: SSR always shows the bar, persisted dismissal takes over client-side without hydration mismatch) | TDD red→green: 3 unit specs (dismiss records, id-scoping, idempotent dismiss) + 1 E2E spec (dismiss hides bar; reload keeps it hidden). Commit `0c4c57d` | **Verified** |
| **R8-8 cross-sell (LOW)** | Deferred with rationale (unchanged since round 7): needs either the merchandiser-curated model (admin UI, FR-809-adjacent) or the conversion-proxy fallback (requires populated analytics events) | Plan §R8-8; PDP header comment still says "cross-sell stub" | **Reasoned** (deferral decision) |

**Non-findings (investigated, recorded to prevent re-chasing):**
- `/cart` promo field EXISTS (`placeholder="Promotion code"` — the sweep probe's `placeholder*=promo` was case-sensitive); the FR-402 gaps that remain are R-CART-1's shipping estimate + inline re-validation notices.
- The repo checkout spec's `.or(getByRole("button", {name: /continue to payment/i}))` masked R8-1: on the placeholder build the SECOND branch matched (form rendered) — after R8-1 the primary branch matches. A dedicated R8-1 spec now pins the notice + no-form state explicitly.
- "const essage" in `newsletter-form.tsx` (Read-tool output) is a display artifact: hex dump proves `const [message, setMessage]`; `tsc --noEmit` exits 0. Same artifact class the repo documents — byte-level verification is authoritative.
- Bestselling sort falls through to featured (schema accepts `?sort=bestselling`, SQL has no branch) — documented R-SHOP-2 deferral, not a new regression.
- New arrivals renders its empty state locally ("New pieces are on the bench") by design at `pageSize 8` vs `is_new` count — catalog-driven, not a defect.

**Gates after round 8:** `pnpm lint` 8/8 · `pnpm typecheck` 8/8 · `pnpm test` 7/7 tasks (config 50, auth 19, admin 11, web 48, db 23, commerce 150 — integration live on embedded PG 17.5; 301 total, +44 vs round 7) · `pnpm build` 2/2 · Playwright web **74/74** local (64 prior + 10 round-8 specs) · admin **8/8** local. `git ls-files | grep '^\.env$'` empty; `skills/` + `infrastructure/` untouched by every commit.
...
## 2026-09-12 — Live E2E round 9 (promo-drop notice R9-1, actionable min-spend copy R9-2, cart shipping estimate R9-3, PLP quick-add R9-4, sticky mobile add-to-cart R9-5)

Environment: fresh clone @ `25ed42d` (main). Embedded PostgreSQL 17.5 (zonky binaries via `embedded-postgres@17.5.0-beta.15`, port 5488, initdb/pg_ctl direct + pgcrypto/pg_trgm, migrate+seed; `DATABASE_URL` exported per-command — dotenv reads CWD and pnpm runs scripts from `packages/db`) — real-PG integration suites + full Playwright suites executed locally against the production build, plus live-origin sweeps of both deployments. Plan + findings: `docs/plans/2026-09-12-live-e2e-remediation-round9.md`.

| Slice | Fix | Evidence | Label |
|---|---|---|---|
| **Live audit round 9** | — | Repo Playwright suite vs live storefront origin: **74/74 PASS** (the round-8 redeploy — checked-in `start_server_log.txt` at HEAD `25ed42d` — published every round-8 fix; the 10 awaiting-redeploy deltas session_14 recorded are closed). Admin gate suite vs live admin origin: **8/8 PASS**. Beyond the repo suite, a 23-probe diagnostic sweep (cart promo/shipping surfaces, PLP card conversion surfaces, PDP mobile bar, JSON-LD/meta completeness, checkout honesty, sitemap parity, headers, TTFB 117–126 ms) surfaced 6 findings — every one validated against the code before classification; probe artifacts (footer newsletter form ≠ checkout address form; canonical probe reading `content` instead of `href` on `<link>`; noindex search page legitimately canonical-less) disproven and recorded | **Verified** |
| **R9-1 promo re-validation drop inline notice (MEDIUM, FR-404 — closes the R-CART-1 notice half)** | `CartDto.promotionNotice` (server-composed, customer-safe: code + humanized reason + automatic re-application hint); `getCartDto` evaluates the attached-but-dropped promotion (at most one — `applyPromotionByCode` replaces the row); cart page renders it as a labelled live region `role="status" aria-label="Promotion notice"` | TDD red→green: 4 integration specs (`cart-promotion-notice.integration.test.ts` — eligible → null, dropped → notice naming code + amounts, re-qualify → cleared, unattached → null) + E2E spec pinning apply → drop → reload → re-apply. Commit `14fdea9` | **Verified** |
| **R9-2 actionable min-spend rejection copy (MEDIUM, FR-402)** | `humanizePromotionRejection(reason, context?)` — with `{minSpendMinor, subtotalMinor, currency}` known, min_spend returns "You're €251.00 away from the €500.00 minimum for this code — add another piece to qualify." (integer minors until display-only Intl; zero-decimal currencies ÷1). All other reasons keep byte-stable copy (equality-pinned). Both rejection sites pass context: `applyPromotionByCode` and the R9-1 notice composer | TDD red→green: 4 new unit specs (amounts named, non-round minor rounding, no-context fallback, byte-stability of other reasons) + strengthened integration/E2E assertions (€251.00 shortfall pinned end-to-end at €249 vs €500). Commit `f699f50` | **Verified** |
| **R9-3 cart shipping estimate by postcode (MEDIUM, FR-402 — closes the R-CART-1 estimate half)** | `commerce/shipping-rates.ts`: the first `ShippingRateProvider` implementation (zone by country via `ANY(countries)`, weight-banded cheapest-per-method, white-glove forcing >30 000 g with the `forced` flag) + `estimateShippingForCart` (cart weight rollup — missing weight counts 0 g; zone-native currency); `estimateShippingAction` (Zod, H1-CART discipline: no cart → `ok(null)`, never mints); `CartShippingEstimate` client island (country select + postcode, 400 ms debounce inside the 500 ms budget, stale-drop by sequence, 3-char mid-typing guard, labelled live region). DISPLAY-ONLY: `shippingMinor` stays null; placement keeps the goods-only decision | TDD red→green: 7 integration specs (EU band, 5–30 kg band, forced white-glove, unknown country, US zone currency, cart rollup, NOT_FOUND) + 6 action specs (validation, no-cart, UUID pass-through, uppercase normalization, error mapping) + E2E (DK estimate then US re-estimate in USD). Promo status regions gained distinct accessible names so strict-mode queries stay unique alongside the new estimate region. Commit `9ae07f5` | **Verified** |
| **R9-4 PLP quick-add (MEDIUM, FR-206 — was UNTRACKED)** | Cards CTE gains a LEFT JOIN LATERAL resolving the first purchasable variant (`is_default DESC, sku ASC` — getProduct's ordering; purchasable = inventory > 0 OR made-to-order via `lead_time_days_max > 7`, the PDP derivation) → `ProductCardDto.quickAddVariantId` (null when every variant is sold out); `QuickAddButton` client island (§8.3 idempotency key, opens the mini-cart drawer, disabled "Sold out" when null) wired on `/shop` + `/shop/{category}` via the ProductCard `quickAdd` slot (outside the card link semantics) | TDD red→green: 4 integration specs (`catalog-quickadd.integration.test.ts` — default-sold-out → alternate variant + default price kept, all-OOS → null, seeded default purchasable, seeded made-to-order) + E2E (enabled on all six seeded cards, click-through opens the drawer with the default-variant price, E2E-2-scoped assertion; count taken BEFORE the drawer opens — the modal marks the page aria-hidden). Commits `612dff2` | **Verified** |
| **R9-5 sticky mobile add-to-cart bar (LOW, FR-309 — was untracked)** | Buy panel observes its primary CTA row (IntersectionObserver, callback-fired setState — R6-2 discipline; hooks declared before the `!selected` early return per rules-of-hooks); when the CTA leaves the viewport a fixed bottom bar renders with the SELECTED variant's live price + an Add-to-cart button invoking the same `onAdd`. `md:hidden`, safe-area padding, `motion-reduce:transform-none`; CTA accessible name carries the price (no strict-mode collision) | TDD red→green: 2 E2E specs at 390×844 (absent initially → appears on scroll → click adds + opens drawer; desktop viewport never renders it). Commit `abb1a92` | **Verified** |
| **R9-6 wishlist (MEDIUM, FR-305 — tracked B9 deferred)** | Deferred with rationale: couples to the B9 account slice (saved-items surface + merge-on-login contract); a localStorage-only heart button would create UI debt the B9 slice immediately rewrites. Stays tracked under R-AUTH-1/B9 with this round's live evidence attached | Plan §R9-6; live probe P05 + `product-buy-panel.tsx` scaffold comment | **Reasoned** (deferral decision) |

**Non-findings (investigated, recorded to prevent re-chasing):**
- The single `input[name=email]` on the placeholder-build `/checkout` is the FOOTER newsletter form (R8-4) — the address form is absent (all forms enumerated; `input[name=line1]` absent). P08's "form field" was a probe artifact.
- Journal/collections canonicals are present and absolute — the sweep probe read `content` instead of `href` on `<link rel=canonical>` (fixed mid-sweep, both PASS).
- `/search` carries no canonical by design (§11.1 noindex page); robots meta asserts `noindex`.
- The empty `[role="status"]` the sweep saw on /cart pre-round-9 was the footer newsletter message container (renders only after submit).
- `?sort=bestselling` shows no active pill — the schema parses it but no SORTS pill exists (R-SHOP-2/product_metrics unchanged deferral).

**Gates after round 9:** `pnpm lint` 8/8 · `pnpm typecheck` 8/8 · `pnpm test` 7/7 tasks (config 50, auth 19, admin 11, web 54, db 23, commerce 169 — integration live on embedded PG 17.5; 326 total, +25 vs round 8) · `pnpm build` 2/2 · Playwright web **79/79** local (74 prior + 5 round-9 specs: 1 notice + 1 estimate + 1 quick-add + 2 sticky-bar) · admin **8/8** local. `git ls-files | grep '^\.env$'` empty; `skills/` + `infrastructure/` untouched by every commit.

**Ops actions required (cannot be executed from this sandbox):**
1. Redeploy both apps via `./start_server.sh` to publish round 9 (no new seed rows or migrations this round — the redeploy only needs the fresh build; migrate + idempotent seed run is unchanged).
2. **Carried from R7-1: rotate `BETTER_AUTH_SECRET` and `CRON_SECRET`** — 4th public exposure; history keeps the bytes.
3. Carried: R-DB-2 DDL hardening as a dedicated session; R-SHOP-2 facets; R-SHOP-3 remainder (verified-buyer gate, moderation admin, +21d review-request job, FR-914 batching); R9-6 wishlist with B9.

**Post-push live re-check (round 9):** web **74/79 vs live** — the 5 remaining failures are exactly the new round-9 specs awaiting redeploy (M1-PROMO amount-bearing copy, R9-1 notice, R9-3 estimate, R9-4 quick-add, R9-5 sticky bar); the R8-6 notify spec additionally fails vs live on the exhausted live-side 3/hour/IP `back_in_stock` bucket (this session's sweep + two full live runs consumed it; it passed vs live earlier in the session and 79/79 locally — environmental, not a regression; the spec's documented fixture reset can only clear the LOCAL bucket). admin **8/8 vs live**. The redeploy (`./start_server.sh`) is the ops action that publishes round 9.
