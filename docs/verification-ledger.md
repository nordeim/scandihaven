...
| **R7-1 .env 4th re-untrack (P0) + R7-1b scan-red prose** | `git rm --cached .env` — commit `a5ffbf7` ("update start server log") re-tracked the 51-line `.env` with real `BETTER_AUTH_SECRET`/`CRON_SECRET` (4th occurrence: C2 → R4-1 → R6-1 → R7-1). Plus `docs/session_8.md:81` quoted the literal OpenSSH key header in prose — the CI scan's `-e '-----BEGIN'` pattern flags it even on a clean checkout | RED: exact CI scan replay flagged `.env:7`, `.env:31`, `session_8.md:81`. GREEN: after untrack + prose rephrase ("OpenSSH private-key header"), a `git archive`-of-staged-index checkout simulation exits **clean** (verified against the index tree, not just the working tree — working-tree `.env` still trips `--no-ignore` scans by design). Rotation of both secrets remains a **required ops action** (4th public exposure) | **Verified** |
