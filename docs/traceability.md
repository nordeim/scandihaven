...
| FR-104 | Typeahead ≥2 chars, ≤10 results | `apps/web/src/app/api/search/typeahead/route.ts` + `commerce/catalog.searchTypeahead`/`searchTypeaheadCategories` + `apps/web/src/components/search-trigger.tsx` (pure shaping in `apps/web/src/lib/search-suggest.ts`) | route Zod contract; rate-limited 60/min/IP; **header combobox Aligned (round 6, 2026-09-11 R6-2)** — 250 ms debounce, ARIA 1.2 listbox, keyboard-navigable, 7 E2E specs + 6 unit assertions; journal group stays reserved until the FR-703 reader route ships (would 404) | **Aligned (round 6)** |
