...
- **Header search is an ARIA combobox** (round 6, R6-2, FR-104): `search-trigger.tsx` (250 ms debounce, abortable fetches, keyboard navigable, dropdown visibility DERIVED — `react-hooks/set-state-in-effect` forbids effect-body setState); the explicit `role="combobox"` overrides `input[type=search]`'s implicit `searchbox` role, so E2E targets `getByRole("combobox")`. Typeahead serves products + categories; the journal group stays empty until the FR-703 reader route exists. `pnpm e2e` covers both apps (admin gate suite is credential-free, R6-3). `NEXT_PUBLIC_*` is inlined at build time — local canonical-spec runs must set `NEXT_PUBLIC_SITE_URL` before building (CI does). Before committing log/doc batches: `git ls-files | grep '^\.env$'` — `316befa` re-tracked it a second time (R6-1).
