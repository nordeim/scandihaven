...
- **Promotion eligibility re-validates per read** (2026-09-10 round 3, E2E-3): attached promos are re-checked against the current subtotal in `getCartDto` and inside the placement TX via `filterEligiblePromotions` — a min-spend code applied above the threshold stops pricing in once the cart drops below it. Keep the `cart_promotion` row (re-crossing re-applies).
- **Card prices come from the default variant** (E2E-4): `COALESCE(MAX(amount) FILTER (WHERE is_default), MIN(amount))` in the cards CTE — a bare `MIN` advertises the cheapest variant while PDP/quick-add price the default.
- **E2E price assertions are scoped** to the order-summary aside or line group (E2E-2: the same amount legitimately renders in line total + Subtotal + Total; page-wide `getByText("€X")` trips Playwright strict mode and left CI's E2E step red).
- **`?redirect=` params go through `@scandihaven/config/redirect-path`** (E2E-5) — the admin previously pushed the raw value into `router.push` (open redirect). Root-level failures render the branded `global-error.tsx` with a one-shot stale-chunk reload via `@scandihaven/config/chunk-recovery` (E2E-1: a rebuild without restart 404s referenced chunks on cached pages).
- **Canonical/OG/sitemap URLs are absolute, resolved per request, SITEWIDE** (round 4, R4-6; round 5, R5-2): the root layout's `generateMetadata` resolves `metadataBase` per request via `@scandihaven/config/site-url` `resolveSiteUrl` (env var → proxy-header-derived served origin → localhost) and every public page declares a canonical + per-page og:url through `apps/web/src/lib/seo.ts` `publicPageMetadata()` (pure, unit-tested). Never emit a metadataBase-relative PDP canonical, and never override `openGraph` on a page with a partial object — Next replaces the segment's openGraph wholesale. `/search?q=` (FR-106) is an RSC page over `listProducts({search})` with `noindex` (§11.1).
- **robots/sitemap assertions are group/parity aware** (round 5, R5-1/R5-3): robots assertions check the `User-agent: *` group, never raw lines (CF-managed per-bot blocks are legitimate); a sitemap-parity spec fetches every advertised `<loc>` — known-empty active categories render a 200 empty state (`hasActiveCategory` seam), unknown slugs 404 (FR-201).
- **Header search is an ARIA combobox** (round 6, R6-2, FR-104): `search-trigger.tsx` (250 ms debounce, abortable fetches, keyboard navigable, dropdown visibility DERIVED — `react-hooks/set-state-in-effect` forbids effect-body setState); the explicit `role="combobox"` overrides `input[type=search]`'s implicit `searchbox` role, so E2E targets `getByRole("combobox")`. Typeahead serves products + categories + journal (since round 7); options navigate via mousedown (no anchor element). `pnpm e2e` covers both apps (admin gate suite is credential-free, R6-3). `NEXT_PUBLIC_*` is inlined at build time — local canonical-spec runs must set `NEXT_PUBLIC_SITE_URL` before building (CI does). Before committing log/doc batches: `git ls-files | grep '^\.env$'` — `316befa` and `a5ffbf7` re-tracked it (R6-1/R7-1; prose quoting key markers trips the scan too — describe, never reproduce, R7-1b).
- **Journal reader routes are category-scoped** (round 7, R7-2, FR-703): `/journal/{category}/{slug}` validates `post.category === params.category` (mismatch 404s); the sitemap and typeahead journal group emit only that URL shape.
- **Client "configured" checks mirror the server rule** (round 8, R8-1): `isStripePublishableKeyConfigured()` (`apps/web/src/lib/stripe-config.ts`) rejects `set-me` placeholders exactly like `getStripe()`; `STRIPE_NOT_CONFIGURED` errors returned to the browser are customer-safe (operator detail logged server-side). PLP/category breadcrumbs emit `BreadcrumbList` JSON-LD like the PDP (R8-3). Review seeds are existence-guarded with honest `isVerifiedPurchase: false` (R8-5). Persisted client visibility (announcement dismiss, R8-7) derives via `useSyncExternalStore` — the M-3 idiom — never `useState` initializers or effect-body setState.
- **Cart status regions are labelled; estimates are display-only** (round 9): the cart page renders multiple `role="status"` live regions — each carries a distinct `aria-label` (`Promotion status`/`Promotion notice`/`Shipping estimate`) so strict-mode locators stay unique. `getCartDto` exposes `promotionNotice` when re-validation drops an attached code (FR-404 inline notice; `humanizePromotionRejection` takes an amounts context for actionable min-spend copy). `estimateShippingForCart` (`commerce/shipping-rates.ts`, the first `ShippingRateProvider` implementation) is DISPLAY-ONLY — `shippingMinor` stays null; placement keeps `shippingMinor: 0`. PLP cards expose `quickAddVariantId` (first purchasable variant — default first, SKU order, PDP purchasable derivation) and the card still advertises the default variant's price (E2E-4); the PDP sticky mobile bar (FR-309) observes the CTA row with IntersectionObserver (hooks declared before any early return).
- **Converted carts never re-resolve; checkout copy is customer-safe end-to-end** (round 10–11): `getCartId()` resolves only ACTIVE carts (a stale cookie after an order behaves like no cookie — the next add mints a fresh cart); `createPaymentIntent` refuses non-active carts with typed `CART_CONVERTED` (before the Stripe call); `placeOrderFromWebhook` no-ops on converted carts (pre-tx + under the FOR UPDATE lock, loud ops-refund log; active/merged still place — never drop a captured payment). The unconfigured-checkout notice is customer-safe in the client static branch too (R10-2 — env-var names, `.env`, restart steps, and test cards never render in the browser). E2E specs assert derived state after deliberate interaction — never an initial absence that only holds pre-hydration (R10-1's ~50 ms race), and DB fixture resets fire only for loopback targets (R10-5). Search synonyms are TITLE-scoped; the original query keeps the full vector contract (R10-3).
- **Admin action hygiene** (round 11): `requirePermission()` runs OUTSIDE the action try (it re-auths by throwing NEXT_REDIRECT — the catch would log it as an error and dead-end expired sessions); multi-table writes run in one `db.transaction` with `FOR UPDATE` on inventory rows and `writeAudit(input, tx)` committing the audit atomically with the change; session-check failures log (`console.error`), never a silent bounce to /sign-in.
- **Search runs on the maintained `product.search_vector`** (round 7, R7-4, R-DB-1): GENERATED ALWAYS STORED (title A / materials+desc B) + GIN; never ad-hoc `to_tsvector` per row. Synonym expansion flows through `expandSearchTerms` (`commerce/src/search-terms.ts`); `listProducts`/`searchTypeahead` conditions stay symmetric. Generated-column gotchas: `to_tsvector` needs the `::regconfig` cast for immutability; `array_to_string` is STABLE (migration 0001's `immutable_text_array_to_string` preamble). Trigram tolerance is `similarity >= 0.5` — full-query typos match, partial two-word typos honestly don't.
- **Workspace vitest configs pin `testTimeout: 30_000`** (round 5, R5-4): suites cold-import the workspace TS graph, which crossed the 5000ms default under 7-way parallel turbo runs on 2-CPU sandboxes (the round-3 "admin guard flake", root-caused). Keep the headroom in new workspaces.
...
| `pnpm e2e` | Playwright Chromium, both apps (web storefront suite + admin credential-free gate suite; needs migrated+seeded DB; `E2E_BASE_URL` to target a running server) |
