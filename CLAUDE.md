...
- **Canonical/OG/sitemap URLs are absolute, resolved per request** (round 4, R4-6): `@scandihaven/config/site-url` `resolveSiteUrl` (env var → proxy-header-derived served origin → localhost) feeds PDP `generateMetadata`, `app/sitemap.ts`, and `app/robots.ts` via `apps/web/src/lib/site-origin.ts` — never emit a metadataBase-relative PDP canonical. SEO entry builders are pure functions in `apps/web/src/lib/seo.ts` (unit-tested); `/search?q=` (FR-106) is an RSC page over `listProducts({search})` with `noindex` (§11.1).
