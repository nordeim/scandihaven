...
| Canonical/OG URLs point at `http://localhost:3000` | `NEXT_PUBLIC_SITE_URL` is unset on the deployment — the declared source of truth; set it to the public origin and redeploy (a production boot logs an actionable `[boot] NEXT_PUBLIC_SITE_URL …` warning while it is misconfigured, E2E-8). Since 2026-09-10 round 4 the canonical/OG/sitemap URLs additionally fall back to the origin each request was served on (`@scandihaven/config/site-url`), and since round 5 (R5-2) that request-scoped resolution covers **every page** (root-layout `metadataBase` + per-page `publicPageMetadata()`), so a missing env var no longer poisons the URL contract — the env var is still required to silence the warning |
| Repo E2E fails on `robots.txt` with "blanket disallow" while the site is fine | Fixed 2026-09-10 (R5-1): production zones legitimately prepend Cloudflare-managed per-bot `Disallow: /` blocks; the robots spec now parses `User-agent:` groups and only forbids a bare `Disallow: /` inside a `*` group |
...
| A category page (`/shop/beds`) 404s though the sitemap lists it | Fixed 2026-09-10 (R5-3): known-active categories with zero products render a 200 empty state ("No pieces here yet"); only unknown/inactive slugs 404 (FR-201). A sitemap-parity E2E now fetches every advertised `<loc>` and fails on any non-200 |
