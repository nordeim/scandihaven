...
- **Cart status live regions carry distinct accessible names** (R9-3): the cart page renders several `role="status"` regions (promo feedback, promo-drop notice, shipping estimate, footer newsletter). Each is labelled (`aria-label="Promotion status" / "Promotion notice" / "Shipping estimate"`) so Playwright strict-mode queries stay unique — a bare `getByRole("status")` on /cart matches multiple elements.
- **Quick-add prices the first purchasable variant** (R9-4, FR-206): the cards CTE resolves `quickAddVariantId` (default first, SKU order, the PDP purchasable derivation — inventory > 0 OR made-to-order via `lead_time_days_max > 7`) — never a bare cheapest-variant pick; the card keeps advertising the DEFAULT variant's price (E2E-4). A modal drawer marks the page `aria-hidden`, so E2E card-button counts must be taken BEFORE opening the drawer.
- **Cart shipping estimates are DISPLAY-ONLY** (R9-3, FR-402): `estimateShippingForCart` (first `ShippingRateProvider` implementation, `commerce/shipping-rates.ts`) resolves the zone by country, bands by cart weight rollup, and forces white-glove above 30 000 g — but never mutates cart totals (`shippingMinor` stays null; placement keeps `shippingMinor: 0`, the documented goods-only decision). Quote amounts are zone-native currency; no FX in Phase 1.
- **Promotion re-validation drops are explained inline** (R9-1, FR-404): `getCartDto` exposes `promotionNotice` when an attached code stops being eligible — server-composed customer-safe copy (code + humanized reason + re-application hint; `humanizePromotionRejection` takes an amounts context since R9-2). New totals paths must keep surfacing it, the same way they must re-validate via `filterEligiblePromotions`.
...
- `docs/plans/2026-09-12-live-e2e-remediation-round9.md` — live E2E round 9 (promo-drop notice R9-1 + actionable min-spend copy R9-2 + postcode shipping estimate R9-3 — R-CART-1 closed; PLP quick-add R9-4; sticky mobile add-to-cart bar R9-5; wishlist R9-6 deferred to B9).
