...
| **HIGH** | Reviews verified-buyer server gate + moderation admin + `+21d` `review_request` job (`delivered_at+21d`) — the back-in-stock CAPTURE half shipped in round 8 (R8-6: `requestBackInStock` service + notify action + PDP form; FR-914 `≤1/day` batching still pending) | Content & retention — unverified reviews pass, no automated solicitation | **Open — P1 (capture half resolved round 8 `49a7683`)** | FR-308, FR-913/914 | `R-SHOP-3` (M) |
| **MEDIUM** | Cart `/cart` shipping estimate 500ms postcode + invalid-promo actionable errors; re-validation inline notices not rendered | ~~P2 friction — cart shows stale eligibility silently~~ | **Resolved — round 9 (2026-09-12), R-CART-1 closed entirely: `R9-1` `CartDto.promotionNotice` renders the inline notice when re-validation drops an attached code (`14fdea9`); `R9-2` min-spend rejection copy names the shortfall and the threshold (`f699f50`); `R9-3` postcode estimate over the first `ShippingRateProvider` implementation (`9ae07f5` — display-only, `shippingMinor` stays null)** | FR-402/404 §8.5 | `R-CART-1` (closed) |
...
| **HIGH** | Checkout placeholder-key blind spot: client `stripePromise` treated any non-empty `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` as configured while the server `getStripe()` rejects `set-me` placeholders — placeholder-key deployments rendered the address form, then leaked `set STRIPE_SECRET_KEY in .env` to the customer (FR-508/509; R8-1) | Money path — dead-end checkout + internal config instructions in the browser | **Resolved — round 8 (`2402ac1`, 2026-09-11): `isStripePublishableKeyConfigured()` (pure, unit-tested) mirrors the server rule client-side; the action logs the operator detail server-side and returns a customer-safe message. E2E pins the honest notice on a placeholder build** | FR-508/509, §9.4 | `R8-1` (closed) |
| **MEDIUM** | Homepage journal preview cards unlinked after the FR-703 reader routes shipped | Highest-traffic page dead-ends into the journal | **Resolved — round 8 (`c663028`, 2026-09-11): cards link `/journal/{category}/{slug}` per the R7-2 URL contract; E2E pins href shape + click-through** | FR-701 §10 | `R8-2` (closed) |
| **MEDIUM** | PLP/category breadcrumbs rendered without BreadcrumbList JSON-LD (PDP only) | FR-201 acceptance gap; rich-result eligibility | **Resolved — round 8 (`fdbd58b`, 2026-09-11): both PLP routes emit `breadcrumbJsonLd` via `safeJsonLd` (PDP escaping idiom); E2E pins trail names + absolute URLs** | FR-201, §11.1 | `R8-3` (closed) |
| **MEDIUM** | Footer missing the FR-107 newsletter form + social icons (the subscribe action's `source` even defaulted to `"footer"` — planned, never rendered) | FR-107 M acceptance gap; signup origin unrecorded | **Resolved — round 8 (`33155a6`, 2026-09-11): `NewsletterForm` gains `source`/`idPrefix`/`compact`; footer carries the compact form + aria-named social links; E2E pins form, success status, names** | FR-107 | `R8-4` (closed) |
| **MEDIUM** | FR-701 §6 brand-story teaser, §7 materials 3-cards, §9 testimonials unimplemented; zero approved reviews existed anywhere, so §9 and the FR-308 PDP read-path could never render; PDP JSON-LD omitted `aggregateRating` (FR-312) | Homepage sections M-priority; reviews surface starved of data | **Resolved — round 8 (`c59ce62`, 2026-09-11): `listApprovedTestimonials` (integration-pinned approved-only), 7 existence-guarded approved review seeds (`isVerifiedPurchase` honestly false), homepage §6/§7/§9 with degrade-to-hide, PDP `aggregateRating` when `ratingCount > 0`; E2E pins all** | FR-701 §6/§7/§9, FR-308, FR-312 | `R8-5` (closed) |
| **MEDIUM** | FR-310 out-of-stock "Notify me" absent — `back_in_stock_request` table + `(variant_id, email)` dedupe index existed but nothing wrote to it; no OOS variant seeded so the surface could not even be exercised | FR-310 M acceptance gap | **Resolved — round 8 (`49a7683`, 2026-09-11): `requestBackInStock` service (dedupe integration-pinned) + notify action (3/hour/IP) + PDP Notify-me row per sold-out variant; seed gains a sold-out Rust variant (additive natural key); FR-914 batching stays with `R-SHOP-3`** | FR-310 | `R8-6` (closed) |
| **LOW** | Announcement bar not dismissible (FR-108 S: "dismissible, persisted via Zustand persist") | FR-108 S acceptance gap | **Resolved — round 8 (`0c4c57d`, 2026-09-11): client island with a persisted Zustand store via `useSyncExternalStore` (M-3 idiom); dismissal keys on the announcement id so new content re-shows; unit + E2E pinned** | FR-108 | `R8-7` (closed) |
| **LOW** | Cross-sell "Pairs well with" absent (curated model or conversion-proxy fallback undecided) | FR-307 S acceptance gap | **Open — queued with rationale (curation model undecided); unchanged since round 7** | FR-307 | `R8-8` (queued) |
| **MEDIUM** | PLP quick-add unwired: `ProductCard` shipped a `quickAdd` slot but no page passed it (planned-never-wired, same class as R8-4's footer) | FR-206 M acceptance gap — untracked in traceability | **Resolved — round 9 (`612dff2`, 2026-09-12): cards CTE LEFT JOIN LATERAL resolves the first purchasable variant (default first, SKU order, PDP purchasable derivation) → `quickAddVariantId`; `QuickAddButton` (§8.3 idempotency, opens drawer, disabled "Sold out" when null) wired on `/shop` + `/shop/{category}`; integration + E2E pinned** | FR-206 §8.8 | `R9-4` (closed) |
| **LOW** | Sticky mobile add-to-cart bar absent (FR-309 S: "appearing after primary CTA scrolls out … price + CTA only; does not overlap footer on short pages") — untracked | FR-309 S acceptance gap | **Resolved — round 9 (`abb1a92`, 2026-09-12): buy-panel IntersectionObserver on the CTA row → fixed bottom bar with the selected variant's live price + Add-to-cart (same handler); `md:hidden`, safe-area, reduced-motion; E2E at 390×844 + desktop negative pinned** | FR-309 | `R9-5` (closed) |
| **MEDIUM** | PDP "Save to wishlist" secondary CTA absent (FR-305 M; guest wishlist localStorage + merge-on-login) | FR-305 M acceptance gap — tracked as B9 deferred | **Open — deferred with rationale (round 9, `R9-6`): couples to the B9 account slice (saved-items surface + merge-on-login contract); a localStorage-only heart button would create UI debt B9 immediately rewrites** | FR-305 §6.1 | `R9-6` (deferred → B9/R-AUTH-1) |
| **CRITICAL→RESOLVED** | `.env` re-committed a 5th time at `fc0379a` ("update start server log") with real `BETTER_AUTH_SECRET`/`CRON_SECRET`; AND the CI secret scan could never go green — AGENTS.md:75 + the verification-ledger R7-1 row quoted the private-key-header marker in prose (round-7/9 docs commits regressed the R7-1b rule while documenting it) | ~~Push-blocking secret exposure + a permanently-red gate masking real signals~~ | **Resolved — round 11 (`b572e20`): .env untracked; both prose sites de-markered; git checkout-index staged-tree simulation scanned with the exact CI patterns exits clean. Rotation of both secrets remains a REQUIRED ops action (5th public exposure)** | PRD §13.3/§13.4 | `R10-6`/`R10-6b` (closed) |
| **HIGH→RESOLVED** | Converted-cart double-charge path: `cart.status='converted'` written at placement but never enforced — addLine, cookie resolution (`getCartId`), `createPaymentIntent`, and placement all ignored it; `/checkout/success` never clears the `sh_cart` cookie. A stale cookie could mint a NEW PaymentIntent (different id — evades the payment unique index) and pay the full old+new line set: a second order, a double charge for the original items | ~~Money path — re-checkout of a converted cart~~ | **Resolved — round 11 (`8bd02bd`): four guards (getCartId resolves only ACTIVE carts — stale cookie behaves as no cookie; createPaymentIntent refuses non-active carts with typed `CART_CONVERTED` before the Stripe call; placement fast-fails pre-tx AND re-checks after the FOR UPDATE lock with a loud ops-refund log; customer-safe action copy). Integration + app-level + action-mapping specs red→green** | FR-508/509 §8.7 | `R10-7` (closed) |
| **HIGH→RESOLVED** | Checkout "not configured" notice rendered operator instructions to customers (env-var names, `.env`, restart steps, test card 4242) — the R8-1 fix sanitized the action path but the client static branch kept the operator copy; live placeholder-key deployments showed it | ~~Money path — internal config instructions in the browser (R8-1 contract gap)~~ | **Resolved — round 10 (`59d423d`): customer-safe notice ("Payments are temporarily unavailable. Please contact us…"); E2E pins zero operator vocabulary in the main region + the order summary staying visible** | FR-508/509 | `R10-2` (closed) |
| **MEDIUM→RESOLVED** | E2E spec quality: the R9-5 sticky-bar spec asserted a false initial state ("CTA in view at load" — it starts ~1098 px down at 390×844) and passed only via a ~50 ms hydration race (bar count 0→1 at t+50 ms), failing full live-suite runs intermittently; the R8-6 back-in-stock spec's fixture reset hard-required a local DB even against live origins (session-16-documented failure class) | ~~CI/spec reliability~~ | **Resolved — round 10 (`41ce57a` + `b1b63b3`): sticky spec asserts observer derivation in BOTH directions (geometry/hydration-independent; product untouched — FR-309 behavior was correct); back-in-stock reset scoped to loopback targets, vs-live runs need no local PG** | PRD §12.1 | `R10-1`/`R10-5` (closed) |
| **MEDIUM→RESOLVED** | Per-page twitter cards absent from `publicPageMetadata` — the root layout default (home title) covered every public page; shared links to /shop, /journal, category PLPs, static pages advertised home copy (PRD §11.1 "per-page … twitter:*") | ~~SEO/social acceptance gap~~ | **Resolved — round 10 (`5325d89`): builder emits `twitter:{card:'summary',title,description}` mirroring the resolved og copy; E2E pins /shop → "Shop all". og:image default deferred (no brand social-card raster asset — design task)** | PRD §11.1 | `R10-4` (closed) |
| **LOW→RESOLVED** | Search relevance: synonym-expanded terms matched B-weight description text (q=lamp surfaced the Hygge throw via "light enough to sleep under"; q=rug surfaced the Øresund lamp via "throws a soft glow") — couch→sofa worked only by coincidence | ~~Search quality~~ | **Resolved — round 10 (`9e92e5e`): `expandSearchTerms` returns `{original, synonyms}`; synonyms match TITLES only (R-DB-1 preserved — no ad-hoc to_tsvector); R7-4 fixture specs stayed green** | FR-105 §8.8 | `R10-3` (closed) |
| **MEDIUM→RESOLVED** | Admin mutation-path debt: `redirect()` (NEXT_REDIRECT) swallowed by both actions' try/catch (expired sessions dead-ended on INTERNAL fail with error-log noise); `updateProductAction` wrote 4+ tables without a transaction (partial state, audit written last and lost on mid-failure) with a read-modify-write inventory delta (lost updates vs webhook movements); silent `.catch(() => null)` on both admin session checks (auth outage indistinguishable from logged-out) | ~~Back-office correctness/observability~~ | **Resolved — round 11 (`7d23d21` + `f82b78e` + `431f504`): requirePermission outside the try; product update is one transaction with FOR UPDATE inventory and an in-tx audit row (writeAudit executor seam, pinned by specs — rollback leaves no phantom audit); session-check failures logged** | PRD §9.2/§8.2 | `A-1`/`A-2`/`A-3`/`A-4` (closed) |
